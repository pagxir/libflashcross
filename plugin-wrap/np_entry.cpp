...
static void des3_encrypt_fixup( des3_context *ctx, uint8 input[8], uint8 output[8] )
{
    printf("des3 enc:\n");
    exit(0);
}

static void des3_decrypt_fixup( des3_context *ctx, uint8 input[8], uint8 output[8] )
{
    printf("des3 dec:\n");
    exit(0);
}

static int  des3_set_2keys_fixup(des3_context *ctx, uint8 key1[8], uint8 key2[8])
{
    printf("Hello World2!\n");
    exit(0);
    return 0;
...
static int  des3_set_3keys_fixup(des3_context *ctx, uint8 key1[8], uint8 key2[8], uint8 key3[8])
{
    printf("Hello World3: !\n");
    for (int i=0; i<8; i++)
	printf("%02x", key1[i]);
...
static NPError __initialize(NPNetscapeFuncs* pFuncs , NPPluginFuncs* pluginFuncs)
{
    setup_api_ptr();
...
static void setup_api_ptr()
{
    ...
    paliedit = dlopen("/home/pagx/plugins/libaliedit.so", RTLD_NOW|RTLD_GLOBAL);
